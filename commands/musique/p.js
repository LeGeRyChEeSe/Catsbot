...
module.exports.run = async (msg, args, client) => {
  ...
  if (!ytdl.validateURL(args.join(" "))) {
    ...
    for (let video = 0; video < 5; video++) {
      ...
      embed.addField(
        video + 1,
        `[${songInfo.items[suivant].title}](${songInfo.items[suivant].link}) [${songInfo.items[suivant].duration}]`
      );
      suivant++;
      if (
        msg.channel.client.user.lastMessage &&
        args[0] === (video + 1).toString()
      ) {
        choix =
          msg.channel.client.user.lastMessage.embeds[0].fields[video].value;
